...
public record UpdateProfileRequest(string Username, string Email, string FullName, string? Bio, string? Phone);
public record AuthResponse(string AccessToken, string RefreshToken, DateTime ExpiresAt, UserDto User);
public record UserDto(Guid Id, string Username, string Email, string FullName, string Role, string? Bio, string? Phone);
