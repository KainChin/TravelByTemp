...
public static class SchemaBootstrapper
{
    ...
    public static async Task EnsureContentSchemaAsync(AppDbContext db, ILogger logger, CancellationToken ct = default)
    {
        ...
        foreach (var sql in CreateStatements)
        {
            ...
            {
                logger.LogWarning(ex, "Skipped schema statement.");
            }
        }
        ...
        {
            logger.LogWarning(ex, "Could not apply column patches (insufficient DB privileges).");
        }

        logger.LogInformation("Content management schema bootstrap finished.");
    }
}
